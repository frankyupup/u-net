...
from nets.original_unet_model import UNet_Origin
...
SAVE_PATH = osp.join("runs", MODEL_NAME)
...
def train_main(dataset_path=DATASET_PATH, epochs=EPOCHS, batch_size=BATCH_SIZE, lr=LEARNING_RATE,
    ...
    focal_lo0_original_unet_train.pyss = False                                                      # 设置为True可解决正负样本不均衡的问题
    cls_weights = np.ones([num_classes], np.float32)                 # 初始化模型训练的损失函数权重
    num_workers = 0
    # window下设置为0即可
    # if MODEL_NAME == "original_unet":
    model = UNet_Origin(n_channels=3, n_classes=num_classes)               # 设置模型输出的通道数目和模型输出的类别数目。为了代码可读性，写明参数的名称是必要的
    ...
    gen = DataLoader(train_dataset, shuffle=True, batch_size=batch_size, num_workers=num_workers, pin_memory=True,      #这个迭代器相关的类的参数含义要注意下，李沐
                     drop_last=True, collate_fn=unet_dataset_collate)
    epoch_step = len(train_lines) // batch_size
    # 获取测试数据集
    if epoch_step == 0:
        raise ValueError("无法加载数据集，请检查：\n1.是否是数据集过小。\n2.你的原图和标签数量和名称是否一致")       #引发异常中断
    ...
    for epoch in range(epochs):
        ...
        fit_one_epoch_no_val(model_train, model, loss_history, optimizer, epoch, epoch_step, gen, epochs, device,
                             dice_loss, focal_loss, cls_weights, num_classes, save_path=save_path, model_save_gap=10)       #即使dice_loss&focal_loss都是false时，使用的是CE_loss
...
if __name__ == '__main__':      #该py是在命令行直接运行的，所以该if的条件成立
