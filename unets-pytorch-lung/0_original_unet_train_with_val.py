...
def train_main(dataset_path=DATASET_PATH, epochs=EPOCHS, batch_size=BATCH_SIZE, lr=LEARNING_RATE,
    ...
    model = UNet_Origin(n_channels=3, n_classes=num_classes)  # 设置模型输出的通道数目和模型输出的类别数目
    ...
    # 获取验证数据集
    val_lines = [x.split(".")[0] for x in os.listdir(osp.join(dataset_path, "Val_Images"))]
    val_dataset = UnetDataset(val_lines, input_shape, num_classes, False, dataset_path, IMAGE_SUFFIX, LABEL_SUFFIX)
    ...
    for epoch in range(epochs):
        ...
        fit_one_epoch(model_train, model, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val,
                      epochs,
                      torch.cuda.is_available(), dice_loss, focal_loss, cls_weights, num_classes, save_path=save_path,
                      model_save_gap=10)
        lr_scheduler.step()
