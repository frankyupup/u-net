...
# BACKBONE_NAME = "vgg16"
# MODEL_PATH = "runs/unet_vgg16/unet_vgg16_model.pth"
BACKBONE_NAME = "resnet50"
MODEL_PATH = "runs/unet_resnet50/unet_resnet50_model.pth"
LABELS_SAVE_PATH = "../results/unet_{}_Results".format(BACKBONE_NAME)
RESULTS_SAVE_PATH = "test_results/unet_{}".format(BACKBONE_NAME)

# 模型验证，三个参数，分别是测试集路径，结果保存路径和实际的标签路径
def val_main(test_dir=osp.join(DATASET_PATH, "Test_Images"), result_dir=LABELS_SAVE_PATH,
             gt_dir=osp.join(DATASET_PATH, "Test_Labels"), model_path=MODEL_PATH, image_suffix = IMAGE_SUFFIX, label_suffix=LABEL_SUFFIX, result_save_path=RESULTS_SAVE_PATH, name_classes=NAME_CLASSES, backbone=BACKBONE_NAME):
    num_classes = len(name_classes)                                             # 数据集的类别数目
    image_ids = [x.split(".")[0] for x in os.listdir(test_dir)]                 # 数据集图片名称
    if not os.path.exists(result_dir):
        os.makedirs(result_dir)
    unet = Unet(model_path=model_path, backbone=backbone)
