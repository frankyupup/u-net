...
def f_score(inputs, target, beta=1, smooth=1e-5, threhold=0.5):
    ...
    # --------------------------------------------#
    #   计算dice系数，这里实际计算是dice，不是f score
    # --------------------------------------------#
...
def compute_mIoU(gt_dir, pred_dir, png_name_list, num_classes, name_classes, image_suffix, label_suffix):
    ...
    # todo 绘制出对应的混淆矩阵以及将更多的分数计算出来，写一个脚本，加载原图和结果图来计算各种分数
    # ------------------------------------------------#
    ...
    print('===> mIoU: ' + str(round(np.nanmean(IoUs) * 100, 2)) + '; mPA: ' + str(
        round(np.nanmean(PA_Recall) * 100, 2)) + '; Accuracy: ' + str(round(per_Accuracy(hist) * 100, 2)) + "; Dice score: {}".format(round(dice_final*100, 3)))
...
def show_results(miou_out_path, hist, IoUs, PA_Recall, Precision, name_classes, tick_font_size=12, plt_show=False, class_names=[]):
    if os.path.isdir(miou_out_path):
        pass
    else:
        os.makedirs(miou_out_path, exist_ok=True)
    draw_plot_func(IoUs, name_classes, "mIoU = {0:.2f}%".format(np.nanmean(IoUs) * 100), "Intersection over Union", \
                   os.path.join(miou_out_path, "mIoU.png"), tick_font_size=tick_font_size, plt_show=plt_show)
    # print("Save mIoU out to " + os.path.join(miou_out_path, "mIoU.png"))

    draw_plot_func(PA_Recall, name_classes, "mPA = {0:.2f}%".format(np.nanmean(PA_Recall) * 100), "Pixel Accuracy", \
                   os.path.join(miou_out_path, "mPA.png"), tick_font_size=tick_font_size, plt_show=plt_show)
    # print("Save mPA out to " + os.path.join(miou_out_path, "mPA.png"))

    draw_plot_func(PA_Recall, name_classes, "mRecall = {0:.2f}%".format(np.nanmean(PA_Recall) * 100), "Recall", \
                   os.path.join(miou_out_path, "Recall.png"), tick_font_size=tick_font_size, plt_show=plt_show)
    # print("Save Recall out to " + os.path.join(miou_out_path, "Recall.png"))

    draw_plot_func(Precision, name_classes, "mPrecision = {0:.2f}%".format(np.nanmean(Precision) * 100), "Precision", \
                   os.path.join(miou_out_path, "Precision.png"), tick_font_size=tick_font_size, plt_show=plt_show)
    ...
    show_heatmaps(title="heatmap", x_labels=class_names, y_labels=class_names, harvest=heat_maps_float,
                  save_name=os.path.join(miou_out_path, "Confusion_matrix.png"))
